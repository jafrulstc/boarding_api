...
import { Person as PersonIcon, CloudUpload, Delete, AttachFile, Emergency } from '@mui/icons-material';
...
import { CreatePersonDTO } from '~/features/education/person/types/personType';
...
const PersonForm = memo(({ person, onSuccess }: PersonFormProps) => {
    ...
    // ⬇️ Memoized selectedCategory and isStaffSelected
    const selectedCategory = useMemo(
        () => personCategories.find(c => c.id === personCategoryId),
        [personCategories, personCategoryId]
    );
    const isStaffSelected = useMemo(
        () => (selectedCategory?.name || '').toLowerCase() === 'staff',
        [selectedCategory]
    );

    // ⬇️ Memoized filteredDesignations
    const filteredDesignations = useMemo(
        () => allDesignations.filter(d => d.designationCategoryId === designationCategoryId),
        [allDesignations, designationCategoryId]
    );
    ...
    }, [designationCategoryId, setValue, isFormInitialized, isEdit, filteredDesignations]);  // Removed watch from dependencies as it's stable
    ...
    const handleFileUpload = async (
        ...
        try {
            ...
            reader.onload = () => {
                ...
                setValue(fieldName, result);  // Removed 'as any'
                if (fileNameField) {
                    setValue(fileNameField, file.name);  // Removed 'as any'
                }
    ...
    const onSubmit = async (data: CreatePersonDTO) => {
        try {
            ...
            if (isStaffSelected) {
                ...
                data.emergencyCont = Number(data.emergencyCont)
            }
            if (isEdit && person) {
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <TextField
                                ...
                                label="emergencyCont"
                                type="number"
                                error={!!errors.emergencyCont}
                                helperText={errors.emergencyCont?.message}
                                value={field.value ?? 0}
                            />
                        )}
                    />
